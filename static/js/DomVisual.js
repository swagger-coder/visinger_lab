...
class DomVisual {
  constructor (bgs) {
    ...
    this.domTextSelector = [
      ...
      '#total-time-minute', '#total-time-second',
      '#current-time-minute', '#current-time-second',
      '#seek-bar','#s-area', '#s-hover', '#ins-time'
    ]
    ...
    this.domControlSelector = [
      ...
      },
      {
        ...
        event: {
          click: () => {
            eventBus.emit('clickBar')
          },
          mousemove: (event) => {           
            let sArea = this.getDom('#s-area', this.domTextMap)
            let sHover = this.getDom('#s-hover', this.domTextMap)
            let insTime = this.getDom('#ins-time', this.domTextMap)
            eventBus.emit('showHover', {event, sArea, sHover, insTime})
          },
          mouseout: () => {
            let sHover = this.getDom('#s-hover', this.domTextMap)
            let insTime = this.getDom('#ins-time', this.domTextMap)
            eventBus.emit('hideHover', { sHover, insTime})
          }
        }
      }
    ]
    this.domContainerSelector = ['#bg', '#info-cover', '#music-lrc', '#setting-wrap', '#alert']
  ...
  removePlayAnimation (dom) {
    let d = dom || this.getControlDom('#submitBtn')
  ...
  handleChange () {
    eventBus.on('change', ({ state, duration, currentTime }) => {
      ...
      if (isNaN(duration) || isNaN(currentTimeFormat)) {
        ...
        this.setDomText('#current-time-minute', this.add0(parseInt(Math.max(Math.floor(currentTimeFormat / 60), 0))))
        this.setDomText('#current-time-second', this.add0(parseInt(Math.max(currentTimeFormat % 60, 0))))
  ...
  alert() {
    console.log('alert');
    this.getContainerDom('#alert').classList.add('show')
    setTimeout(() => {
      this.getContainerDom('#alert').classList.remove('show')
    }, 2000)
  }
}